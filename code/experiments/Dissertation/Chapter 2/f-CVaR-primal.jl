...
vline!([VaR_30], linestyle=:dash, color =:magenta, linewidth=2, label="z* = 30% VaR[x̃₁]")
plot!([VaR_30,CVaR_30], [.15,.15], arrow=:left, linewidth=2, label="-E[z*-x̃₁]₊/0.3", color =:plum2)
plot!(x, pdf.(left_tail_dist, x), label="x̃₂ = Tail Distribution of x̃₁", linewidth=3, color=:darkblue,alpha=0.7)
vline!([CVaR_30], linestyle=:dash, color =:dodgerblue2, linewidth=2, label="30% CVaR[x̃₁] = z*-E[z*-x̃₁]₊/0.3 = E[x̃₂]")
